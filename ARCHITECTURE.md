...
Task Management 当前实现 `app/platform/task` 的状态机、Attempt/Event 生命周期、命令幂等基础、PostgreSQL 生命周期持久化、受信任服务端生产者提交能力、独立 Worker 执行边界、恢复/取消/重试调度以及主体隔离的 Task HTTP 查询与控制接口。一次合法领取以单条 `TASK_CLAIMED` 事件同时记录领取和开始执行；安全 `TaskView` 不含 lease 或输入指纹，含 lease 的契约仅供受信任执行器内部消费。受信任提交从已认证主体确定 owner，并由 Composition 固定 task type、尝试限制、重试策略和展示字段白名单；它不是公开 HTTP 或浏览器创建入口。Task HTTP 只允许已认证主体查询自己的 Task、读取脱敏事件、协作取消和按策略手动重试；路由通过 Application/Port 执行 owner admission，不暴露创建、领取、续租、结果回写、恢复调度或 lease。PostgreSQL Repository 将 Task、Attempt、Event 和命令回执在同一事务中保存，并以唯一约束、外键、行锁、`SKIP LOCKED` 候选选择和 active Attempt/lease 到期索引保护持久化不变量。Worker 通过固定 task type 执行器绑定轮询、续租和安全结果回写；当前 Tender Worker 由 `python -m app.run_tender_task_worker` 独立进程显式启动，依次运行恢复、重试和 Tender 领取阶段，每阶段使用独立 Session/Composition 资源，HTTP 生命周期不会启动它。RecoveryCoordinator、RetryScheduler 和 CancellationCoordinator 只通过 Application/Port 触发过期恢复、退避重入队、协作取消和受策略约束的手动重试；内存仓储仅存在于 `tests/task/` 作为验证替身。Tender 接入、前端、E2E 和 Workflow 的后续扩展仍必须以独立 Change 交付；当前 Workflow 只交付后端 Run/Node 契约，详见 4.11。
...
执行策略是 Agent Management 内部边界，不是客户端可选参数。策略只能接收服务端重新读取的能力条目、固定 `dispatch_key`、标准化输入和可信主体上下文。未登记异步档案的能力继续由同步策略处理；已登记能力由 Agent→Task 桥接通过既有受信任 Task 提交 Application 创建固定策略的 Task，并返回不透明执行引用。桥接只保存输入指纹和安全展示摘要，不保存原始输入、凭据或 lease；它不提供通用 HTTP、MCP 或浏览器 Task 创建入口。内部 Dialogue 收到异步策略的 `accepted` 后，将受控 `execution_reference` 写入 `agent_call` 事件并交给 Chat；这只是“已接收”状态，不是完成结果，也不会触发 continuation 或 assistant Message。TM-07.4 的 Tender Consumer 通过主体绑定的 Attachment 快照读取器消费 `tender.generate_bid_skeleton`，使用固定 Executor、lease 续租和协作取消；外部 Tender MCP 使用独立的同步 Dispatcher，并在当前请求内返回 `EmbeddedResource`，内部 Task 结果不映射为浏览器下载。
...
当前 `app/platform/workflow` 只建立后端运行事实，不实现完整编排产品。`WorkflowVersion` 是 Composition Root 固定注册的不可变有限 DAG，节点只能引用能力目录中的 `capability_code`，不能携带 URL、Python 类名、客户端 `dispatch_key` 或任意执行器。注册时校验节点、边、无环性以及能力输入/输出契约；当前使用 Tender `generate_bid_skeleton` 作为固定能力节点样本。
