...
当前处于迁移期的例外是尚未完成的 Phase 3：由于已有阶段文档和 OpenSpec 归档仍不完整，Phase 3 暂以 `docs/第三阶段工作计划.md` 为联合蓝本，并以一份前后端联合进度文档管理执行证据；已经建立的 Chat 流式 Change 仍按 OpenSpec 流程收口。Phase 3 结束后，这两份阶段文档一并删除，后续阶段不再新建同类阶段拆分计划文档，而从 OpenSpec Explore/Change 开始管理详细交付。
...
| Phase 3：单 Agent 真实业务调用 | 进行中 | 以投标助手为首个垂直验证场景，完成单个 Agent 经 LangChain 调用 LLM 并引用 RAG 知识的前后端闭环。 |
...
Phase 3 的终点不是完成一套完整投标工作台，而是验证一个真实、可同步调用的单 Agent 闭环：
...
- 将招标文件写入政策知识库。
- 投标书结构生成、正文生成、人工确认工作流，或把现有 Mock 任务页接入真实接口。
...
在 Phase 3 验证单 Agent 真实调用稳定后，下一阶段聚焦平台的核心差异化能力：让多个职责明确的 Agent 可被组合、调度和观察，从而承载更复杂的业务流程。
...
投标助手可以在平台能力成熟后继续演进，也可以接入其他业务 Agent。诸如招标要求提取、投标书结构生成、内容生成、人工确认等能力，均应作为独立业务需求评估，并通过各自的 OpenSpec Change 推进。

## 5. OpenSpec 使用原则

1. 出现新业务能力或新模块需求时，可先使用 `openspec-explore` 澄清问题与边界；探索不是 Change，也不直接实现代码。
2. Phase 3 剩余工作继续以联合蓝本和前后端联合进度文档推进，不强行补建新的 Phase 3 Change；已存在的 Chat 流式 Change 仍按 OpenSpec 流程同步和归档。
