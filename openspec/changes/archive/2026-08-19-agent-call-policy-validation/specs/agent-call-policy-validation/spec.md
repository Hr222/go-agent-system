...
#### Scenario: Agent 调用输入符合目录契约

- **WHEN** 调用输入满足目录声明的字段、必填资料和 JSON 类型
- **THEN** 系统继续评估确认策略

#### Scenario: Agent 调用输入不符合目录契约
