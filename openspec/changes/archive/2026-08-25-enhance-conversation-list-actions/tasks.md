...
- [x] 2.5 为置顶写入增加每个 `owner_subject` 最多 10 个的服务端策略：目标状态幂等、取消置顶不受限、置顶不更新 `updated_at`，并以事务级主体锁防止并发突破上限。
- [x] 2.6 将置顶上限映射为受控 `409` 响应，补充上限、幂等、时间戳保持、主体隔离与并发配额的 PostgreSQL/HTTP 测试。

## 3. Chat 前端菜单与状态

- [x] 3.1 扩展会话列表 API、类型和 React Query hooks，支持置顶、删除和列表缓存失效；补充 API/hook 测试。
- [x] 3.2 接入会话项重命名菜单与编辑保存/清除，完成失败反馈和菜单关闭。
- [x] 3.3 接入单项删除菜单、二次确认、成功清理和失败提示。
- [x] 3.4 接入置顶/取消置顶菜单，刷新列表排序并显示操作反馈；第 11 个置顶失败时保留当前列表并显示受控提示；分享仅保留无动作占位项。
- [x] 3.5 调整侧栏滚动与菜单定位，确保编辑区、菜单和删除确认不被裁切；补充组件交互测试。

## 4. 验证与人工验收

- [x] 4.1 运行后端 Conversation/架构测试、前端 `npm run test -- --run` 和 `npm run build`。
- [x] 4.2 运行 `openspec validate enhance-conversation-list-actions --strict --no-interactive` 与全库严格校验，并执行 `git diff --check`。
- [x] 4.3 在本地 Chat 页面按“重命名、删除、置顶”顺序人工验收，并确认分享占位项点击无任何效果及越权失败提示。
