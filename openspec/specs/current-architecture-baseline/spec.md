...
- **AND** `ARCHITECTURE.md` MUST 标明 PostgreSQL 生命周期持久化和受信任 Worker 已实现，而租约恢复与公开协议边界仍未实施
