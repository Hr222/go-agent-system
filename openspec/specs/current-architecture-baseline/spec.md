...
- **AND** `ARCHITECTURE.md` MUST 标明 PostgreSQL 生命周期持久化、受信任 Worker 和租约恢复/取消/重试调度已实现，而 HTTP 管理接口与其他公开协议边界仍未实施
