...
定义在 Conversation 中安全执行一次已授权结构化 Agent 调用、持久化受控结果事件并向页面返回安全摘要的稳定契约。

## Requirements

### Requirement: 对话可以执行一次已授权的结构化 Agent 调用

系统 MUST 在可信主体和 Conversation 校验通过后，将结构化 `AgentCall` 交给 P2.6 受控分发服务。只有分发结果为成功时才视为 Agent 已执行；确认缺失、策略拒绝、目录不可用或运行时失败时 MUST 返回受控状态且不得重复调用 Agent Runtime。
...
系统 MUST 在本能力中只保存用户请求和 Agent 事件，不自动调用普通 Chat LLM 生成最终自然语言助手消息。Agent 结果必须保留关联 ID，供后续 Dialogue Agent Continuation 使用。
