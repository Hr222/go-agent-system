...
系统 MUST 在 `POST /api/v1/interaction/chat/stream` 中先完成候选召回、结构化识别、可信主体权限过滤、目录读取和输入复核，再依据服务端目录的确认策略决定下一步。浏览器 MUST NOT 通过能力代码、确认策略、权限或分发键选择执行分支。
...
#### Scenario: 无法识别或需要补充资料

- **WHEN** 识别结果为澄清、未识别、拒绝或失败
